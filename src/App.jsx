        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="flex items-center cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
              ...
              {/* LOGO SCALAR - Ruta directa a public/logo.png */}
              <img 
                src="/logo.png" 
                alt="Logo SCALAR" 
            ...
            <div className="hidden md:flex space-x-8 items-center">
              <button onClick={() => scrollToSection('problema')} className="text-slate-300 hover:text-white transition-colors text-sm font-medium">El Problema</button>
              <button onClick={() => scrollToSection('solucion')} className="text-slate-300 hover:text-white transition-colors text-sm font-medium">Nuestros Agentes</button>
              <button onClick={() => scrollToSection('testimonios')} className="text-slate-300 hover:text-white transition-colors text-sm font-medium">Clientes</button>
              <button onClick={() => scrollToSection('precios')} className="text-slate-300 hover:text-white transition-colors text-sm font-medium">Planes</button>
              <button 
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-slate-900 border-b border-slate-800">
            <div className="px-4 pt-2 pb-6 space-y-2">
              <button onClick={() => scrollToSection('problema')} className="block w-full text-left py-3 text-slate-300">El Problema</button>
              <button onClick={() => scrollToSection('solucion')} className="block w-full text-left py-3 text-slate-300">Agentes</button>
              <button onClick={() => scrollToSection('testimonios')} className="block w-full text-left py-3 text-slate-300">Clientes</button>
              <button onClick={() => scrollToSection('precios')} className="block w-full text-left py-3 text-slate-300">Planes</button>
              <button onClick={() => scrollToSection('demo')} className="block w-full text-center mt-4 bg-indigo-600 py-3 rounded-lg font-bold">Agendar Demo</button>
            </div>
          </div>
        )}
      </nav>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[300px]">
            <div className="md:col-span-2 bg-slate-900 rounded-3xl p-8 border border-slate-800 hover:border-indigo-500/50 transition-colors group relative overflow-hidden flex flex-col justify-between">
              ...
              <div className="mt-4 flex items-center text-sm font-medium text-indigo-400">
                ...
                <span className="text-emerald-400 flex items-center"><Zap className="w-3 h-3 mr-1"/> Activo 24/7</span>
              </div>
            </div>
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="space-y-4">
            {[
              ...
              { q: "¿Es seguro para mis datos financieros?", a: "Totalmente. Usamos seguridad de nivel bancario. El agente de finanzas solo ve lo que tú le permites y te ayuda a tener orden, pero tú siempre tienes el control final del dinero." }
            ].map((item, index) => (
          ...
          <div className="bg-slate-900 p-8 rounded-3xl border border-slate-700 shadow-2xl max-w-md mx-auto">
            <form className="space-y-4 text-left">
              <div>
                ...
                <input type="text" className="w-full bg-slate-950 border border-slate-700 rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all" placeholder="Tu nombre" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-1">Correo Corporativo</label>
                <input type="email" className="w-full bg-slate-950 border border-slate-700 rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all" placeholder="nombre@empresa.com" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-1">Teléfono</label>
                <input type="tel" className="w-full bg-slate-950 border border-slate-700 rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent outline-none transition-all" placeholder="+56 9..." />
              </div>
              <button type="button" className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold py-4 rounded-lg shadow-lg shadow-indigo-500/25 transition-all transform hover:scale-[1.02]">
                Agendar Demo Gratis
              </button>
